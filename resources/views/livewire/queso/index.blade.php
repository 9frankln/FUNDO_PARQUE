<x-recent-record-host :active="$recentRecord !== null" class="space-y-6">
        ...
        <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
            ...
            @if(auth()->user()->tienePermiso('queso', 'crear'))
                <a href="{{ route('queso.create') }}" class="agro-button w-full sm:w-auto">
                    <span>➕</span> Registrar Elaboración
                </a>
            @endif
        </div>
    </div>
    ...
    <section wire:ignore x-data="cheeseDashboard(@js($dashboardData))" x-cloak
             class="overflow-hidden rounded-[1.75rem] border border-emerald-950/10 bg-white shadow-[0_24px_70px_-38px_rgba(6,78,59,0.35)] dark:border-emerald-200/10 dark:bg-slate-950/75 dark:shadow-black/30">
        <div class="relative overflow-hidden border-b border-emerald-950/10 bg-gradient-to-br from-emerald-50 via-white to-sky-50 px-4 py-5 dark:border-emerald-200/10 dark:from-emerald-950/70 dark:via-slate-950 dark:to-sky-950/50 sm:px-6">
            ...
            <div class="relative flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="text-xs text-slate-500 dark:text-slate-400">Actualizado {{ $dashboardData['generatedAt'] }}</span>
                    </div>
                    <h2 class="mt-2 text-xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-2xl">Radiografía de la producción de queso</h2>
                    <p class="mt-1 max-w-3xl text-sm text-slate-600 dark:text-slate-400">Tendencias, mezcla de presentaciones y comportamiento histórico sin consultas adicionales al cambiar de gráfico.</p>
                </div>
                <div class="flex w-full flex-col gap-2 sm:flex-row sm:items-end xl:w-auto">
                    <div class="block min-w-52 relative" 
                        ...
                        <span class="mb-1 block text-[10px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400">Analizar un mes específico</span>
                        <button x-ref="trigger" type="button" @click="dropdownOpen = !dropdownOpen"
                                class="flex h-10 w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3.5 text-left text-xs font-bold text-slate-700 shadow-sm transition focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 hover:border-emerald-400 dark:hover:border-emerald-400"
                            ...
                            <svg class="h-4 w-4 shrink-0 text-slate-400 transition-transform" :class="dropdownOpen && 'rotate-180'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <template x-teleport="body">
                            <div x-show="dropdownOpen" @click.outside="dropdownOpen = false"
                                 ...
                                 :style="menuStyle"
                                 class="z-50 mt-1.5 max-h-60 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-950/15 dark:border-slate-700 dark:bg-slate-900"
                                 style="display: none;">
                                <button type="button" @click="selectPeriod(''); dropdownOpen = false"
                                        :class="selectedPeriod === '' ? 'bg-emerald-600 text-white dark:bg-emerald-400/20 dark:text-emerald-50' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'"
                                ...
                                <template x-for="month in [...visibleMonths].reverse()" :key="`option-${month.period}`">
                                    <button type="button" @click="selectPeriod(month.period); dropdownOpen = false"
                                            :class="selectedPeriod === month.period ? 'bg-emerald-600 text-white dark:bg-emerald-400/20 dark:text-emerald-50' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'"
                                            class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-xs font-bold transition">
                    ...
                    <div class="inline-flex items-center gap-1 rounded-2xl border border-slate-200/90 bg-white/90 p-1.5 shadow-sm backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/90" aria-label="Alcance temporal del dashboard">
                        <template x-for="item in [{v: 6, l: '6 meses'}, {v: 12, l: '12 meses'}, {v: 24, l: '24 meses'}]" :key="item.v">
                            <button type="button" @click="setRange(item.v)"
                                    :class="range === item.v 
                                        ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-md shadow-emerald-600/25 ring-1 ring-emerald-400/30 dark:from-emerald-500 dark:to-teal-400 dark:text-slate-950 font-extrabold scale-[1.02]' 
                                        : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800/80 font-bold'"
                                ...
                                <span class="h-1.5 w-1.5 rounded-full transition-all duration-300" 
                                      :class="range === item.v ? 'bg-white dark:bg-slate-950 animate-pulse' : 'bg-transparent'"></span>
        ...
        <div class="space-y-4 p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                <h3 class="font-bold text-slate-900 dark:text-white">Evolución mensual</h3>
                            </div>
                        ...
                        <div class="inline-flex w-fit rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-700 dark:bg-slate-950">
                            <button type="button" @click="metric = 'weight'; activePoint = null"
                                    :class="metric === 'weight' ? 'bg-emerald-600 text-white dark:bg-emerald-400 dark:text-emerald-950' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'"
                            ...
                            <button type="button" @click="metric = 'units'; activePoint = null"
                                    :class="metric === 'units' ? 'bg-sky-600 text-white dark:bg-sky-400 dark:text-sky-950' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'"
                                    class="rounded-md px-3 py-1.5 text-[11px] font-bold transition">Moldes</button>
                        </div>
                    </div>

                    <div class="mt-4 overflow-hidden rounded-xl border border-slate-200/80 bg-white px-2 pt-2 dark:border-slate-800 dark:bg-slate-950/70">
                        ...
                        <div class="grid pb-2" :style="`grid-template-columns: repeat(${visibleMonths.length}, minmax(0, 1fr))`">
                            <template x-for="(month, index) in visibleMonths" :key="`label-${month.period}`">
                                <span class="truncate text-center text-[9px] font-semibold text-slate-400" x-text="showMonthLabel(index) ? month.label : ''"></span>
                            </template>
                        </div>
                    </div>
                    <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs">
                        <span class="text-slate-500 dark:text-slate-400">Mes de referencia: <strong class="text-slate-800 dark:text-slate-200" x-text="currentMonth?.fullLabel || 'Sin datos'"></strong></span>
                        <span :class="monthlyChange !== null && monthlyChange > 0 ? 'text-emerald-700 dark:text-emerald-300' : (monthlyChange !== null && monthlyChange < 0 ? 'text-rose-700 dark:text-rose-300' : 'text-slate-500')"
                              class="rounded-full bg-slate-100 px-2.5 py-1 font-bold dark:bg-slate-800" x-text="changeText()"></span>
                    </div>
                </article>
                ...
                <article class="rounded-2xl border border-sky-200/90 bg-sky-50/45 p-4 shadow-sm xl:col-span-4 dark:border-sky-500/20 dark:bg-sky-500/[.055] sm:p-5">
                    <h3 class="font-bold text-slate-900 dark:text-white">Mezcla de presentaciones</h3>
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Cantidad de moldes por tamaño en el periodo.</p>
                    <div class="mt-5 flex flex-col items-center gap-5 sm:flex-row xl:flex-col 2xl:flex-row">
                        <div class="relative h-40 w-40 shrink-0 rounded-full shadow-inner" :style="`background: ${donutBackground}`">
                            <div class="absolute inset-[18%] flex flex-col items-center justify-center rounded-full border border-slate-200 bg-white text-center dark:border-slate-700 dark:bg-slate-900">
                                <strong class="text-2xl text-slate-900 dark:text-white" x-text="format(presentationTotal)"></strong>
                                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">moldes</span>
                            </div>
                        </div>
                            ...
                            <template x-for="item in presentationData" :key="item.weight">
                                <div>
                                    <div class="flex items-center justify-between gap-3 text-xs">
                                        <span class="flex min-w-0 items-center gap-2 font-semibold text-slate-700 dark:text-slate-300"><i class="h-2.5 w-2.5 shrink-0 rounded-full" :style="`background:${item.color}`"></i><span class="truncate" x-text="item.label"></span></span>
                                        <span class="shrink-0 font-bold text-slate-900 dark:text-white"><span x-text="format(item.quantity)"></span> · <span x-text="`${format(item.percentage, 1)}%`"></span></span>
                                    </div>
                                    <div class="mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800"><div class="h-full rounded-full" :style="`width:${item.percentage}%; background:${item.color}`"></div></div>
                                </div>
                            </template>
                            <p x-show="presentationData.length === 0" class="rounded-xl border border-dashed border-slate-300 p-4 text-center text-xs text-slate-500 dark:border-slate-700">Sin desglose de presentaciones en este periodo.</p>
                        </div>
                    </div>
                </article>
                ...
                <article class="rounded-2xl border border-amber-200/90 bg-amber-50/45 p-4 shadow-sm xl:col-span-7 dark:border-amber-500/20 dark:bg-amber-500/[.055] sm:p-5">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            <h3 class="font-bold text-slate-900 dark:text-white">Comparación anual</h3>
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Hasta seis años de historia productiva.</p>
                        </div>
                        <div class="flex rounded-lg bg-slate-200/70 p-1 dark:bg-slate-800">
                            <button type="button" @click="annualMetric = 'weight'" :class="annualMetric === 'weight' ? 'bg-white text-emerald-700 shadow-sm dark:bg-slate-700 dark:text-emerald-300' : 'text-slate-500'" class="rounded-md px-2.5 py-1 text-[10px] font-bold">kg</button>
                            <button type="button" @click="annualMetric = 'units'" :class="annualMetric === 'units' ? 'bg-white text-sky-700 shadow-sm dark:bg-slate-700 dark:text-sky-300' : 'text-slate-500'" class="rounded-md px-2.5 py-1 text-[10px] font-bold">moldes</button>
                        </div>
                    </div>
                    <div class="mt-5 space-y-3">
                        <template x-for="year in annualRows" :key="year.year">
                            <div class="grid grid-cols-[3.5rem_minmax(0,1fr)_6.5rem] items-center gap-3">
                                <span class="text-sm font-extrabold text-slate-700 dark:text-slate-300" x-text="year.year"></span>
                                <div class="h-7 overflow-hidden rounded-lg bg-slate-200/80 dark:bg-slate-800">
                                ...
                                <strong class="text-right text-xs text-slate-800 dark:text-slate-200" x-text="metricText(annualValue(year), annualMetric)"></strong>
                            </div>
                        </template>
                        <p x-show="annualRows.length === 0" class="py-8 text-center text-xs text-slate-500">Aún no hay información anual.</p>
                    </div>
                </article>
                ...
                <article class="rounded-2xl border border-violet-200/90 bg-violet-50/45 p-4 shadow-sm xl:col-span-5 dark:border-violet-500/20 dark:bg-violet-500/[.055] sm:p-5">
                    <h3 class="font-bold text-slate-900 dark:text-white">Ritmo por día de semana</h3>
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Kilos elaborados y jornadas activas según el día.</p>
                    <div class="mt-5 flex h-48 items-end justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3 pb-3 pt-6 dark:border-slate-800 dark:bg-slate-950/70">
                        <template x-for="day in weekdayRows" :key="day.key">
                            <div class="flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-2" :title="`${day.label}: ${format(day.weight, 2)} kg en ${day.days} jornada(s)`">
                                <span class="text-[9px] font-bold text-slate-500" x-text="day.weight > 0 ? format(day.weight, 1) : ''"></span>
                                ...
                                <span class="text-[10px] font-extrabold text-slate-600 dark:text-slate-400" x-text="day.label"></span>
                            </div>
                        </template>
                    </div>
                </article>
    ...
    <x-collapsible-filters :active="$hasActiveFilters"
        ...
        <div class="grid grid-cols-1 items-end gap-3 border-t border-slate-100 pt-4 dark:border-zinc-800/60 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            <div class="xl:col-span-2">
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Buscar</label>
                <div class="relative w-full">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 text-sm text-slate-400 dark:text-zinc-500">&#x1F50D;</span>
                    <input type="search" wire:model.live.debounce.300ms="search"
                           placeholder="Escribe un dato para buscar..."
                           class="w-full rounded-xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 py-1.5 pl-10 pr-4 text-sm text-slate-800 dark:text-zinc-300 outline-none transition placeholder:text-slate-400 dark:placeholder:text-zinc-650 focus:border-emerald-500/50 focus:ring-emerald-500/20 focus:ring-2">
                </div>
            </div>
            @if($tab === 'diario')
                <div>
                    <label class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Acceso rápido</label>
            ...
            @endif
            <div>
                <label class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Año</label>
            ...
            @if($tab !== 'anual')
                <div>
                    <label class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Mes</label>
                ...
                <div>
                    <label class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Desde</label>
                    <input type="date" wire:model.live="fechaDesde"
                           class="w-full rounded-xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 px-3 py-1.5 text-sm text-slate-800 dark:text-zinc-200 outline-none transition focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/20">
                </div>
                <div>
                    <label class="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Hasta</label>
        ...
        <div class="overflow-x-auto rounded-2xl bg-zinc-900 border border-zinc-800 shadow-md">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-zinc-850/60 text-sm text-zinc-300">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-12 text-center text-zinc-550">
                ...
                <tbody class="divide-y divide-zinc-850/60 text-sm text-zinc-300">
                    @forelse($produccionesSemanales as $sem)
                        <tr class="hover:bg-zinc-850/20 transition duration-200">
                            ...
                            <td class="p-4 text-zinc-550 whitespace-nowrap">{{ $sem->dias_producidos }} días</td>
    ...
    <!-- TAB 3: MENSUAL -->
    @if($tab === 'mensual')
        <div class="overflow-x-auto rounded-2xl border border-violet-200 bg-white shadow-md dark:border-violet-500/25 dark:bg-slate-950/70">
            <table class="w-full border-collapse text-left">
                ...
                <tbody class="divide-y divide-violet-100 text-sm text-slate-700 dark:divide-violet-500/15 dark:text-slate-300">
                    @forelse($produccionesMensuales as $mesProduccion)
                        <tr class="transition odd:bg-white even:bg-violet-50/70 hover:bg-violet-100/70 dark:odd:bg-slate-950/20 dark:even:bg-violet-500/[.055] dark:hover:bg-violet-500/10">
                            <td class="p-4 whitespace-nowrap">
    ...
    <!-- TAB 4: ANUAL -->
    @if($tab === 'anual')
        <div class="overflow-x-auto rounded-2xl border border-amber-200 bg-white shadow-md dark:border-amber-500/25 dark:bg-slate-950/70">
            <table class="w-full border-collapse text-left">
                <thead>
                ...
                <tbody class="divide-y divide-amber-100 text-sm text-slate-700 dark:divide-amber-500/15 dark:text-slate-300">
                    @forelse($produccionesAnuales as $anioProduccion)
                        <tr class="transition odd:bg-white even:bg-amber-50/70 hover:bg-amber-100/70 dark:odd:bg-slate-950/20 dark:even:bg-amber-500/[.055] dark:hover:bg-amber-500/10">
            ...
            <div role="dialog" aria-modal="true" aria-labelledby="queso-report-title"
                ...
                <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-4 py-4 sm:px-6 dark:border-slate-700">
                    <div class="min-w-0">
                        ...
                        <h3 id="queso-report-title" class="text-lg font-bold text-slate-900 dark:text-white sm:text-xl">Generar reporte PDF de producción de queso</h3>
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Selecciona contenido y campos. Se aplicarán los filtros activos en la tabla.</p>
                    </div>
                ...
                <div class="min-h-0 flex-1 overflow-y-auto xl:grid xl:grid-cols-[21rem_minmax(0,1fr)] xl:overflow-hidden">
                    <aside class="border-b border-slate-200 bg-slate-50/70 p-4 xl:overflow-y-auto xl:border-b-0 xl:border-r dark:border-slate-700 dark:bg-slate-950/25">
                        <div class="mb-3 flex items-center justify-between gap-2">
                            <span class="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Secciones del reporte</span>
                        ...
                        <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-1">
                            @foreach($reportOptions as $key => $option)
                                ...
                                <label wire:key="queso-report-section-{{ $key }}"
                                       :class="selectedSections.includes('{{ $key }}') ? '{{ $theme['selected'] }}' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300 dark:hover:border-slate-600'"
                                    ...
                                    <span :class="selectedSections.includes('{{ $key }}') ? '{{ $theme['check'] }}' : 'border-slate-300 bg-white dark:border-slate-600 dark:bg-slate-900'"
                    ...
                    <div class="p-4 sm:p-5 xl:overflow-y-auto 2xl:p-6">
                        <div x-show="selectedSections.length === 0" class="flex min-h-48 items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 text-center dark:border-slate-700 dark:bg-slate-950/30">
                            <div class="max-w-sm">
                                <strong class="block text-sm text-slate-700 dark:text-slate-200">Sin secciones seleccionadas</strong>
                                <span class="mt-1 block text-xs text-slate-500 dark:text-slate-400">Activa una sección para elegir sus campos.</span>
                            </div>
                        </div>
                                    ...
                                    <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-3">
                                        @foreach($fields as $fieldKey => $fieldLabel)
                                            <label wire:key="queso-report-field-{{ $sectionKey }}-{{ $fieldKey }}"
                                                   :class="(reportColumns['{{ $sectionKey }}'] || []).includes('{{ $fieldKey }}') ? '{{ $theme['field'] }}' : 'border-slate-200 bg-white/75 text-slate-600 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-400 dark:hover:border-slate-600'"
                                                ...
                                                <span :class="(reportColumns['{{ $sectionKey }}'] || []).includes('{{ $fieldKey }}') ? '{{ $theme['check'] }}' : 'border-slate-300 bg-white dark:border-slate-600 dark:bg-slate-950'"
                ...
                <div class="flex flex-col-reverse gap-2 border-t border-slate-200 bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 dark:border-slate-700 dark:bg-slate-900">
                    <p class="hidden text-xs text-slate-500 sm:block dark:text-slate-400">A4 horizontal · Filtros actuales · Secciones y campos configurables</p>
