<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-start gap-4">
            ...
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.18em] text-emerald-400">Registro de producción</p>
        ...
        @if(auth()->user()->tienePermiso('queso', 'actualizar'))
            <a href="{{ route('queso.edit', $produccion->id) }}"
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:items-start">
        <div class="space-y-6 lg:col-span-2">
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="rounded-2xl border border-teal-500/20 bg-teal-500/10 p-5">
                    ...
                    <span class="block mt-1 text-3xl font-black text-teal-400">{{ number_format((float) $produccion->peso_total_kg, 2) }}</span>
            ...
            <section class="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900">
                <div class="border-b border-zinc-800 px-5 py-4">
                    <h2 class="text-sm font-bold uppercase tracking-wider text-emerald-400">Distribución por presentación</h2>
                </div>
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-zinc-800/70 text-sm">
                                @foreach($produccion->presentaciones as $presentacion)
                                    <tr>
                                        ...
                                        <td class="px-5 py-4 text-right font-bold text-teal-400">{{ number_format($presentacion->subtotal_kg, 2) }} kg</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot class="border-t border-zinc-700 bg-zinc-950/60 text-sm font-bold">
                                <tr>
                                    ...
                                    <td class="px-5 py-4 text-right text-emerald-400">{{ number_format((float) $produccion->peso_total_kg, 2) }} kg</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
            ...
            @if($produccion->observaciones)
                <section class="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
                    <h2 class="text-xs font-bold uppercase tracking-wider text-emerald-400">Observaciones</h2>
        ...
        <aside class="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <h2 class="text-xs font-bold uppercase tracking-wider text-emerald-400">Foto del lote</h2>
