<div x-data="{ showPassword: false, showConfirmation: false }"
    ...
    <section role="dialog" aria-modal="true" aria-label="Asignar nueva contraseña" class="agro-dialog agro-dialog--compact">
        <header class="flex items-start justify-between border-b border-zinc-200 p-4 dark:border-zinc-800 sm:px-6">
            <div><p class="agro-kicker">Seguridad de cuenta</p><h3 class="mt-1 text-xl font-extrabold">Nueva contraseña</h3><p class="mt-1 text-xs text-zinc-500">Usuario: {{ $passwordResetUserName }}</p></div>
        ...
        <div class="space-y-4 p-4 sm:p-6">
            ...
            <label class="block"><span class="mb-1.5 block text-xs font-bold">Contraseña nueva</span><div class="relative"><input wire:model="newPassword" x-bind:type="showPassword ? 'text' : 'password'" autocomplete="new-password" class="w-full rounded-xl border border-zinc-300 px-4 py-2.5 pr-11 text-sm dark:border-zinc-700"><button type="button" x-on:click="showPassword = !showPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-500" x-text="showPassword ? 'Ocultar' : 'Ver'"></button></div>@error('newPassword')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror</label>
            <label class="block"><span class="mb-1.5 block text-xs font-bold">Confirmar contraseña</span><div class="relative"><input wire:model="newPasswordConfirmation" x-bind:type="showConfirmation ? 'text' : 'password'" autocomplete="new-password" class="w-full rounded-xl border border-zinc-300 px-4 py-2.5 pr-11 text-sm dark:border-zinc-700"><button type="button" x-on:click="showConfirmation = !showConfirmation" class="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-500" x-text="showConfirmation ? 'Ocultar' : 'Ver'"></button></div>@error('newPasswordConfirmation')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror</label>
        </div>
