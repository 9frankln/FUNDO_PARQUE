...
<head>
    ...
    <meta name="description" content="Conoce {{ $publicFundoName }}: ganadería, equinos, infraestructura y procesos de trabajo rural.">
    <meta name="theme-color" content="#071a13">
    <title>{{ $publicFundoName }} | {{ $branding->name }}</title>

    @if($heroItems->isNotEmpty())
        <link rel="preload" as="image" href="{{ $heroItems->first()['full'] }}" fetchpriority="high">
    @endif
...
>
    <header class="landing-header" :class="!atTop && 'is-scrolled'">
        <div class="landing-container landing-navbar">
            ...
            <nav class="landing-nav-links" aria-label="Navegación principal">
                ...
                @foreach($contentBlocks as $section => $block)
                    <a href="#{{ $section }}">{{ \App\Models\LandingBlock::sectionDefinitions()[$section]['label'] }}</a>
                @endforeach
                @if($galleryItems->isNotEmpty())<a href="#galeria">Galería</a>@endif
            </nav>
        ...
        <div x-cloak x-show="mobileNavOpen" x-transition class="landing-mobile-menu xl:hidden">
            <div class="landing-container">
                <nav aria-label="Navegación móvil">
                    ...
                    @foreach($contentBlocks as $section => $block)
                        <a href="#{{ $section }}" @click="mobileNavOpen = false">{{ \App\Models\LandingBlock::sectionDefinitions()[$section]['label'] }}</a>
                    @endforeach
                    @if($galleryItems->isNotEmpty())<a href="#galeria" @click="mobileNavOpen = false">Galería</a>@endif
                ...
                <div style="padding-bottom: 1rem">
                    @auth
                        <a href="{{ route('dashboard') }}" class="landing-access" style="width: 100%">Abrir dashboard</a>
                    @else
    ...
    <main id="inicio" class="landing-main scroll-mt-24">
        ...
        <section class="landing-container landing-hero">
            <div class="landing-hero-copy">
                ...
                <p class="landing-kicker">{{ $heroSettings['eyebrow'] ?: 'Producción rural conectada' }}</p>
                <h1>{{ $heroTitle }}</h1>
                @if($showFundoName && filled($branding->tagline))<span class="landing-hero-fundo">{{ $branding->tagline }}</span>@elseif($showFundoName)<span class="landing-hero-fundo">{{ $publicFundoName }}</span>@endif
                <p>{{ $heroContent }}</p>

                <div class="landing-hero-actions">
                    <a href="{{ $primaryTarget }}" class="landing-primary">{{ $heroSettings['cta_label'] ?: 'Conocer el fundo' }}<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-width="2" d="m9 18 6-6-6-6"/></svg></a>
                    @if($galleryItems->isNotEmpty())<a href="#galeria" class="landing-secondary">Ver galería<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-width="1.8" d="M4 7h16v12H4V7Zm3-3h10v3H7V4Zm1 8 2.5 2.5L14 11l3 4"/></svg></a>@endif
                </div>

                <div class="landing-hero-facts {{ $showLocationFact ? '' : 'is-compact' }}" aria-label="Resumen público">
                    <div><strong>{{ $contentBlocks->count() }}</strong><span>áreas principales</span></div>
                    <div><strong>{{ $galleryItems->count() }}</strong><span>registros visuales</span></div>
                    @if($showLocationFact)<div class="landing-hero-location"><strong>{{ $locationFactTitle }}</strong>@if($locationFactDetail)<span>{{ $locationFactDetail }}</span>@endif</div>@endif
                </div>
            </div>
        ...
        <section id="areas" class="landing-areas scroll-mt-24" aria-labelledby="areas-title">
            ...
            <div class="landing-area-grid">
                @foreach($contentBlocks as $section => $block)
                    ...
                    @endphp
                    <a href="#{{ $section }}" class="landing-area-card">
                        @if($cover)<img class="landing-framed-image" style="--media-focus-x: {{ $coverFrame['focus_x'] }}%; --media-focus-y: {{ $coverFrame['focus_y'] }}%; --media-zoom: {{ $coverFrame['zoom'] }}" src="{{ $cover->hasGeneratedConversion('thumb') ? $cover->getUrl('thumb') : $cover->getUrl() }}" alt="" loading="lazy" decoding="async">@endif
                        <span><small>{{ str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT) }}</small><span><strong>{{ filled($block->title) ? $block->title : $defaults['title'] }}</strong><em>Explorar área</em></span></span>
                    </a>
                @endforeach
            </div>
        </section>
        ...
        @foreach($contentBlocks as $section => $block)
            ...
            @endphp
            <section id="{{ $section }}" class="landing-section landing-story {{ $loop->even ? 'is-reversed' : '' }} scroll-mt-24" aria-labelledby="{{ $section }}-title">
                <div class="landing-story-grid">
                    <div class="landing-story-media" x-data="{ images: @js($sectionImages), open(index) { $dispatch('open-lightbox', { images: this.images, captions: this.images.map(() => @js(filled($block->title) ? $block->title : $defaults['title'])), current: index }) } }">
                        @forelse($media as $image)
                            @php
                                $imageFrame = \App\Models\LandingBlock::mediaFrame($image);
                            @endphp
                            <figure><img class="landing-framed-image" style="--media-focus-x: {{ $imageFrame['focus_x'] }}%; --media-focus-y: {{ $imageFrame['focus_y'] }}%; --media-zoom: {{ $imageFrame['zoom'] }}" src="{{ $image->hasGeneratedConversion($loop->first ? 'optimized' : 'thumb') ? $image->getUrl($loop->first ? 'optimized' : 'thumb') : $image->getUrl() }}" alt="{{ $image->getCustomProperty('caption') ?: (filled($block->title) ? $block->title : $defaults['title']) }}" loading="lazy" decoding="async"><button type="button" @click="open({{ $loop->index }})" aria-label="Ampliar imagen"></button></figure>
                        @empty
                    ...
                    <div class="landing-story-copy">
                        <span class="landing-story-number">{{ str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT) }}</span>
                        @if(filled($settings['eyebrow'] ?? null))<p class="landing-kicker">{{ $settings['eyebrow'] }}</p>@endif
                        <h2 id="{{ $section }}-title">{{ filled($block->title) ? $block->title : $defaults['title'] }}</h2>
                        <p>{{ filled($block->content) ? $block->content : $defaults['content'] }}</p>
                        @if($features->isNotEmpty())<div class="landing-feature-list">@foreach($features as $feature)<span>{{ $feature }}</span>@endforeach</div>@endif
                        @if($galleryItems->isNotEmpty())<a href="#galeria" class="landing-story-link">Ver registro visual<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-width="2" d="m9 18 6-6-6-6"/></svg></a>@endif
                    </div>
                </div>
            </section>
        @endforeach
        ...
        <x-gallery
            :items="$galleryItems"
            :title="filled($galleryBlock?->title) ? $galleryBlock->title : $galleryDefaults['title']"
            :content="filled($galleryBlock?->content) ? $galleryBlock->content : $galleryDefaults['content']"
            :eyebrow="$gallerySettings['eyebrow']"
        />
    </main>

    <footer id="pie" class="landing-footer scroll-mt-24">
        <div class="landing-container landing-footer-grid {{ $showLocationFact ? '' : 'is-compact' }}">
            <div><h3>{{ $publicFundoName }}</h3><p>{{ $heroContent }}</p></div>
            <div><h4>Explorar</h4><nav><a href="#inicio">Inicio</a>@foreach($contentBlocks as $section => $block)<a href="#{{ $section }}">{{ \App\Models\LandingBlock::sectionDefinitions()[$section]['label'] }}</a>@endforeach@if($galleryItems->isNotEmpty())<a href="#galeria">Galería</a>@endif</nav></div>
            @if($showLocationFact)<div><h4>Ubicación</h4>@if($publicLocation)<p>{{ $publicLocation }}</p>@endif@if($publicAddress)<p>{{ $publicAddress }}</p>@endif</div>@endif
        </div>
        <div class="landing-container landing-footer-bottom"><span>&copy; {{ date('Y') }} {{ $publicFundoName }}. Información pública institucional.</span><span>Gestionado con {{ $branding->name }}</span></div>
    </footer>
    ...
    <div class="landing-floating-actions">
        ...
        @if($whatsAppUrl)
            <a href="{{ $whatsAppUrl }}" target="_blank" rel="noopener noreferrer" class="landing-whatsapp" aria-label="Contactar por WhatsApp">
