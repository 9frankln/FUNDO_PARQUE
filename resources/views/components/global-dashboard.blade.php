...
<div
    wire:key="global-dashboard-{{ $data['refreshKey'] }}"
        ...
        <div class="relative grid gap-8 p-6 sm:p-8 xl:grid-cols-[1.45fr_.8fr] xl:p-10">
            <div class="max-w-3xl">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <span class="agro-dashboard-hero__date text-xs font-semibold text-zinc-500 dark:text-zinc-400">{{ $data['welcome']['date'] }}</span>
                </div>
                ...
                <div class="mt-5 flex flex-wrap items-center gap-3 text-xs font-semibold text-zinc-600 dark:text-zinc-300">
                    ...
                    <span class="agro-dashboard-hero__context inline-flex items-center gap-2 rounded-xl border border-zinc-200 bg-white/90 px-3 py-2 text-zinc-700 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/90 dark:text-zinc-300">
                        ...
                        Datos actualizados a las {{ $data['generatedAt'] }}
                    </span>
                </div>

                <div class="mt-7 flex flex-wrap gap-3">
                    @if($data['createPermissions']['animal'] ?? false)
                        <a href="{{ route('animal.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-extrabold text-white shadow-md shadow-emerald-600/20 transition hover:-translate-y-0.5 hover:bg-emerald-700 dark:bg-emerald-500 dark:text-slate-950 dark:hover:bg-emerald-400">
            ...
            <div class="agro-dashboard-hero__quick self-stretch rounded-2xl border border-emerald-200/80 bg-white/90 p-5 shadow-md backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-900/95">
                ...
                <div class="mt-5 grid grid-cols-3 gap-2 xl:grid-cols-1">
                    @if($data['allowedModules']['monitoreo'] ?? false)
                        <a href="{{ route('monitoreo.index') }}" class="agro-dashboard-hero__metric rounded-xl border border-zinc-200/80 bg-slate-50/90 p-3 shadow-sm transition hover:bg-white dark:border-zinc-800 dark:bg-zinc-950/90 dark:hover:bg-zinc-950 xl:flex xl:items-center xl:justify-between">
                            <span class="block text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Alertas pendientes</span>
                            <strong class="mt-1 block text-2xl font-black {{ ($data['kpis']['alerts']['overdue'] ?? 0) > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-zinc-950 dark:text-white' }}">
                                {{ $data['kpis']['alerts']['pending'] }}
                            </strong>
                        </a>
                    @endif
                    @if($data['allowedModules']['leche'] ?? false)
                        <a href="{{ route('leche.index') }}" class="agro-dashboard-hero__metric rounded-xl border border-zinc-200/80 bg-slate-50/90 p-3 shadow-sm transition hover:bg-white dark:border-zinc-800 dark:bg-zinc-950/90 dark:hover:bg-zinc-950 xl:flex xl:items-center xl:justify-between">
                            <span class="block text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Ordeño Hoy</span>
                            <strong class="mt-1 block text-2xl font-black text-cyan-700 dark:text-cyan-400">{{ number_format($data['kpis']['milk']['today'], 1) }} <small class="text-xs">L</small></strong>
                        </a>
                    @endif
                    @if($data['allowedModules']['finanzas'] ?? false)
                        <a href="{{ route('finanzas.index') }}" class="agro-dashboard-hero__metric rounded-xl border border-zinc-200/80 bg-slate-50/90 p-3 shadow-sm transition hover:bg-white dark:border-zinc-800 dark:bg-zinc-950/90 dark:hover:bg-zinc-950 xl:flex xl:items-center xl:justify-between">
                            <span class="block text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Saldo del mes</span>
                            <strong class="mt-1 block text-lg font-black {{ $data['kpis']['finance']['balance'] >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}">
    ...
    <section aria-labelledby="dashboard-summary-title">
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            @if($data['allowedModules']['animal'] ?? false)
                <a href="{{ route('animal.index') }}" class="group rounded-2xl border border-sky-200/70 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-sky-400/20 dark:bg-zinc-900/80">
                    <div class="flex items-start justify-between">
                        ...
                        <span class="text-xs font-bold text-sky-700 dark:text-sky-300">+{{ $data['kpis']['animals']['newThisMonth'] }} este mes</span>
                    </div>
                    <p class="mt-4 text-3xl font-black text-zinc-950 dark:text-white">{{ $data['kpis']['animals']['total'] }}</p>
                    <p class="mt-1 text-xs font-bold text-zinc-500 dark:text-zinc-400">Animales en inventario</p>
                    <p class="mt-3 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500">{{ $data['kpis']['animals']['inactive'] }} dados de baja en el historial</p>
                </a>
            @endif

            @if($data['allowedModules']['leche'] ?? false)
                <a href="{{ route('leche.index') }}" class="group rounded-2xl border border-cyan-200/70 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-cyan-400/20 dark:bg-zinc-900/80">
                    <div class="flex items-start justify-between">
                        ...
                        <span class="rounded-full px-2.5 py-1 text-[10px] font-extrabold {{ $data['kpis']['milk']['variation'] >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300' }}">
                            {{ $data['kpis']['milk']['variation'] >= 0 ? '+' : '' }}{{ $data['kpis']['milk']['variation'] }}%
                        </span>
                    </div>
                    <p class="mt-4 text-3xl font-black text-zinc-950 dark:text-white">{{ number_format($data['kpis']['milk']['month'], 1) }} <small class="text-sm text-zinc-400">L</small></p>
                    <p class="mt-1 text-xs font-bold text-zinc-500 dark:text-zinc-400">Leche producida este mes</p>
                    <p class="mt-3 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500">Promedio 7 días: {{ number_format($data['kpis']['milk']['average7Days'], 1) }} L/día</p>
                </a>
            @endif

            @if($data['allowedModules']['queso'] ?? false)
                <a href="{{ route('queso.index') }}" class="group rounded-2xl border border-amber-200/70 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-amber-400/20 dark:bg-zinc-900/80">
                    <div class="flex items-start justify-between">
                        ...
                        <span class="rounded-full px-2.5 py-1 text-[10px] font-extrabold {{ $data['kpis']['cheese']['variation'] >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300' }}">
                            {{ $data['kpis']['cheese']['variation'] >= 0 ? '+' : '' }}{{ $data['kpis']['cheese']['variation'] }}%
                        </span>
                    </div>
                    <p class="mt-4 text-3xl font-black text-zinc-950 dark:text-white">{{ number_format($data['kpis']['cheese']['monthKg'], 1) }} <small class="text-sm text-zinc-400">kg</small></p>
                    <p class="mt-1 text-xs font-bold text-zinc-500 dark:text-zinc-400">Queso producido este mes</p>
                    <p class="mt-3 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500">{{ $data['kpis']['cheese']['monthUnits'] }} unidades registradas</p>
                </a>
            @endif

            @if($data['allowedModules']['finanzas'] ?? false)
                <a href="{{ route('finanzas.index') }}" class="group rounded-2xl border border-emerald-200/70 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-emerald-400/20 dark:bg-zinc-900/80">
                    <div class="flex items-start justify-between">
                        ...
                        <span class="rounded-full px-2.5 py-1 text-[10px] font-extrabold {{ $data['kpis']['finance']['balance'] >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300' }}">
                            {{ $data['kpis']['finance']['balance'] >= 0 ? 'Saldo positivo' : 'Saldo negativo' }}
                        </span>
                    </div>
                    <p class="mt-4 text-3xl font-black {{ $data['kpis']['finance']['balance'] >= 0 ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300' }}">S/ {{ number_format($data['kpis']['finance']['balance'], 0) }}</p>
                    <p class="mt-1 text-xs font-bold text-zinc-500 dark:text-zinc-400">Balance de caja del mes</p>
                    <p class="mt-3 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500">Ingresos S/ {{ number_format($data['kpis']['finance']['income'], 0) }} · Egresos S/ {{ number_format($data['kpis']['finance']['expense'], 0) }}</p>
                </a>
            @endif

            @if($data['allowedModules']['engorde'] ?? false)
                <a href="{{ route('engorde.index') }}" class="group rounded-2xl border border-lime-200/70 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-lime-400/20 dark:bg-zinc-900/80">
                    <div class="flex items-start justify-between">
                        ...
                        <span class="text-xs font-bold text-lime-700 dark:text-lime-300">{{ $data['kpis']['fattening']['lots'] }} lotes</span>
                    </div>
                    <p class="mt-4 text-3xl font-black text-zinc-950 dark:text-white">{{ $data['kpis']['fattening']['active'] }}</p>
                    <p class="mt-1 text-xs font-bold text-zinc-500 dark:text-zinc-400">Animales en engorde activo</p>
                    <p class="mt-3 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500">{{ $data['kpis']['fattening']['ready'] }} listos para venta</p>
                </a>
            @endif

            @if($data['allowedModules']['monitoreo'] ?? false)
                <a href="{{ route('monitoreo.index') }}" class="group rounded-2xl border border-rose-200/70 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-rose-400/20 dark:bg-zinc-900/80">
                    <div class="flex items-start justify-between">
                        ...
                        @if($data['kpis']['alerts']['overdue'] > 0)
                            <span class="rounded-full bg-rose-50 px-2.5 py-1 text-[10px] font-extrabold text-rose-700 dark:bg-rose-400/10 dark:text-rose-300">{{ $data['kpis']['alerts']['overdue'] }} vencidas</span>
                        @else
                    ...
                    <p class="mt-4 text-3xl font-black text-zinc-950 dark:text-white">{{ $data['kpis']['alerts']['pending'] }}</p>
                    <p class="mt-1 text-xs font-bold text-zinc-500 dark:text-zinc-400">Alertas sanitarias pendientes</p>
                    <p class="mt-3 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500">{{ $data['kpis']['alerts']['today'] }} programadas para hoy</p>
                </a>
            @endif
        </div>
    </section>
        ...
        @if($data['allowedModules']['monitoreo'] ?? false)
            <section class="rounded-[1.75rem] border border-zinc-200/80 bg-white p-5 shadow-sm sm:p-6 dark:border-zinc-800 dark:bg-zinc-900/80">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('monitoreo.index') }}" class="text-[11px] font-extrabold text-emerald-700 hover:text-emerald-600 dark:text-emerald-300">Ver monitoreo →</a>
                </div>

                <div class="mt-5 space-y-2">
                    @forelse($data['priorities'] as $priority)
                        <a href="{{ route('monitoreo.index') }}" class="flex items-start gap-3 rounded-xl border border-zinc-100 p-3 transition hover:border-emerald-200 hover:bg-emerald-50/40 dark:border-zinc-800 dark:hover:border-emerald-500/30 dark:hover:bg-emerald-950/30">
                            <span class="mt-1 h-2.5 w-2.5 shrink-0 rounded-full {{ $priority['status'] === 'overdue' ? 'bg-rose-500' : ($priority['status'] === 'today' ? 'bg-amber-400' : 'bg-sky-400') }}"></span>
                            <span class="min-w-0 flex-1">
                                <span class="flex items-center justify-between gap-3">
                                    <strong class="truncate text-xs text-zinc-900 dark:text-white">{{ $priority['type'] }}</strong>
                                    <small class="shrink-0 text-[10px] font-extrabold {{ $priority['status'] === 'overdue' ? 'text-rose-600 dark:text-rose-300' : 'text-zinc-400' }}">{{ $priority['date'] }}</small>
                                </span>
                                <span class="mt-1 block line-clamp-1 text-[11px] font-semibold text-zinc-500 dark:text-zinc-400">{{ $priority['message'] }}</span>
                                @if($priority['animal'])
                                    <span class="mt-1 block text-[10px] font-bold text-emerald-700 dark:text-emerald-300">{{ $priority['animal'] }}</span>
                                @endif
                            </span>
                        </a>
                    @empty
        ...
        <section class="rounded-[1.75rem] border border-zinc-200/80 bg-white p-5 shadow-sm sm:p-6 dark:border-zinc-800 dark:bg-zinc-900/80">
            ...
            <div class="mt-5 grid gap-4 md:grid-cols-2">
                <div class="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4 dark:border-emerald-900/40 dark:bg-emerald-950/30">
                    ...
                    <div class="space-y-1">
                        @if($data['allowedModules']['animal'] ?? false)
                            <a href="{{ route('animal.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @endif
                        @if($data['allowedModules']['engorde'] ?? false)
                            <a href="{{ route('engorde.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @endif
                        @if($data['allowedModules']['leche'] ?? false)
                            <a href="{{ route('leche.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @endif
                        @if($data['allowedModules']['queso'] ?? false)
                            <a href="{{ route('queso.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @endif
                        @if($data['allowedModules']['monitoreo'] ?? false)
                            <a href="{{ route('monitoreo.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                ...
                <div class="rounded-2xl border border-teal-100 bg-teal-50/50 p-4 dark:border-teal-900/40 dark:bg-teal-950/30">
                    ...
                    <div class="space-y-1">
                        @if($data['allowedModules']['finanzas'] ?? false)
                            <a href="{{ route('finanzas.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @if(auth()->user()->tienePermiso('buscador', 'leer'))
                            <a href="{{ route('buscador') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @if(auth()->user()->tienePermiso('ajustes', 'leer'))
                            <a href="{{ route('ajustes.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
                        ...
                        @endif
                        @if(auth()->user()->tienePermiso('auditoria', 'leer'))
                            <a href="{{ route('auditoria.index') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-white dark:hover:bg-zinc-800">
