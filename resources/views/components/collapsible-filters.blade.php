...
<div x-data="{ filtersOpen: @js((bool) $active) }"
     {{ $attributes->class(['space-y-4 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition-colors sm:p-6 dark:border-zinc-800 dark:bg-zinc-900']) }}>
    <div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div>
            <h2 class="text-sm font-bold text-zinc-900 dark:text-zinc-100">{{ $title }}</h2>
            @if($description)
