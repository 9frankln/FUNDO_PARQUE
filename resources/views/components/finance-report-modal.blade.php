        ...
        <div class="flex items-start justify-between gap-4 border-b border-zinc-200 px-4 py-4 sm:px-6 dark:border-zinc-800">
            <div>
                <div class="mb-1 flex items-center gap-2">
                    <span class="text-[10px] font-bold uppercase tracking-[0.18em] {{ $accentText }}">Exportación PDF</span>
                    <span class="rounded-full bg-zinc-100 px-2 py-0.5 text-[10px] font-semibold text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
                        <span x-text="sections.length"></span> de {{ count($sectionOptions) }} secciones
                    </span>
                </div>
                <h3 id="finance-report-title" class="text-lg font-bold text-zinc-900 dark:text-white sm:text-xl">{{ $title }}</h3>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto xl:grid xl:grid-cols-[21rem_minmax(0,1fr)] xl:overflow-hidden">
            <aside class="border-b border-zinc-200 bg-zinc-50/70 p-4 xl:overflow-y-auto xl:border-b-0 xl:border-r dark:border-zinc-800 dark:bg-zinc-950/25">
                <div class="mb-3 flex items-center justify-between gap-3">
                    ...
                    <button type="button" x-on:click="sections = sections.length === {{ count($sectionOptions) }} ? [] : @js(array_keys($sectionOptions))"
                            class="text-xs font-bold {{ $accentText }}">
                        <span x-text="sections.length === {{ count($sectionOptions) }} ? 'Limpiar todas' : 'Seleccionar todas'"></span>
                    </button>
                </div>
                <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-1">
                    @foreach($sectionOptions as $sectionKey => $section)
                        @php
                            $theme = $sectionThemes[$sectionKey] ?? $sectionThemes['summary'];
                        @endphp
                        <label :class="sections.includes('{{ $sectionKey }}') ? '{{ $theme['selected'] }}' : 'border-zinc-200 bg-white text-zinc-700 hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900/70 dark:text-zinc-300 dark:hover:border-zinc-700'"
                            ...
                            <input type="checkbox" x-model="sections" value="{{ $sectionKey }}" class="sr-only">
                            <span :class="sections.includes('{{ $sectionKey }}') ? '{{ $theme['check'] }}' : 'border-zinc-300 bg-white dark:border-zinc-700 dark:bg-zinc-900'"
                                  class="flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 transition">
                                <svg x-cloak x-show="sections.includes('{{ $sectionKey }}')" class="h-3 w-3 text-white" fill="none" stroke="currentColor" stroke-width="4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m5 12 4 4L19 6" /></svg>
                            </span>
                            <span class="min-w-0 leading-tight">
                                <strong class="block text-sm">{{ $section['label'] }}</strong>
                                <small class="mt-0.5 block text-[11px] font-normal opacity-65">{{ $section['description'] }}</small>
                            </span>
                        </label>
                    @endforeach
                </div>
                @error('selectedReportSections') <p class="mt-2 text-xs font-medium text-rose-500">{{ $message }}</p> @enderror
                @error('selectedReportSections.*') <p class="mt-2 text-xs font-medium text-rose-500">{{ $message }}</p> @enderror
            </aside>
                ...
                <div class="grid content-start gap-4 xl:grid-cols-2 2xl:gap-5">
                    @foreach($sectionOptions as $sectionKey => $section)
                        @php
                            ...
                            $theme = $sectionThemes[$sectionKey] ?? $sectionThemes['summary'];
                        @endphp
                        <section x-cloak x-show="sections.includes('{{ $sectionKey }}')" class="rounded-2xl border p-4 {{ $theme['panel'] }}">
                            <div class="mb-3 flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <strong class="block text-sm {{ $theme['title'] }}">Campos: {{ $section['label'] }}</strong>
                                    <small class="mt-0.5 block text-[11px] text-zinc-500 dark:text-zinc-400">Elige información incluida en esta sección.</small>
                                </div>
                                <button type="button" x-on:click="columns['{{ $sectionKey }}'] = columns['{{ $sectionKey }}'].length === {{ count($fields) }} ? [] : @js(array_keys($fields))"
                                        class="shrink-0 text-[11px] font-bold transition focus:outline-none focus:underline {{ $theme['action'] }}">
                                    <span x-text="(columns['{{ $sectionKey }}'] || []).length === {{ count($fields) }} ? 'Limpiar todos' : 'Seleccionar todos'"></span>
                                </button>
                            </div>
                            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-3">
                                @foreach($fields as $fieldKey => $fieldLabel)
                                    <label :class="(columns['{{ $sectionKey }}'] || []).includes('{{ $fieldKey }}') ? '{{ $theme['field'] }}' : 'border-zinc-200 bg-white/75 text-zinc-600 hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900/70 dark:text-zinc-400 dark:hover:border-zinc-700'"
                                        ...
                                        <span :class="(columns['{{ $sectionKey }}'] || []).includes('{{ $fieldKey }}') ? '{{ $theme['check'] }}' : 'border-zinc-300 bg-white dark:border-zinc-700 dark:bg-zinc-950'"
                                              class="flex h-5 w-5 shrink-0 items-center justify-center rounded border transition">
                                            <svg x-cloak x-show="(columns['{{ $sectionKey }}'] || []).includes('{{ $fieldKey }}')" class="h-3 w-3 text-white" fill="none" stroke="currentColor" stroke-width="4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m5 12 4 4L19 6" /></svg>
                                        </span>
                                        <span>{{ $fieldLabel }}</span>
                                    </label>
                                @endforeach
                            </div>
                            @error("reportColumns.{$sectionKey}") <p class="mt-2 text-xs font-medium text-rose-500">{{ $message }}</p> @enderror
                            @error("reportColumns.{$sectionKey}.*") <p class="mt-2 text-xs font-medium text-rose-500">{{ $message }}</p> @enderror
                        </section>
                    @endforeach
                </div>
